...
export default function Navbar() {
  ...
  return (
    <React.Fragment>
      {loaded && (
        <div
          className={`flex fixed top-0 z-[999] items-center justify-center ${
            isMobile()
        ...
        >
          <div
            ref={menuRef}
            className={`lg:w-[56.25rem] md:w-[40.813rem] w-[21.875rem] ${
              isHamburgerMenu ? "h-max" : "h-[3.5rem]"
            } rounded-[2.75rem] flex ${
              isHamburgerMenu ? "flex-col" : "flex-row"
            } items-center lg:justify-between bg-background backdrop-blur-[0.563rem] ${
              isHamburgerMenu
                ? `${
          ...
          >
            ...
            <div
              ...
              ${isHamburgerMenu && "opacity-0 animate-fade-in"}`}
            >
              {navItems.map((item) => (
                <div
                  key={item.id}
                ...
                >
                  ...
                  <div
                    className={`absolute bottom-0 h-[0.125rem] rounded-[0.688rem] w-0 ${
                      selectedNav === item.id
                        ? "w-[calc(100%-3.563rem)]"
                        : "w-0"
              ...
              <div
                className={`${
                  isHamburgerMenu
                    ...
                    : "max-sm:hidden max-md:hidden max-lg:hidden"
                } syneFont bg-white text-black font-bold leading-[1.2em] tracking-[0em] p-[0.563rem_2.063rem] rounded-[1.875rem] cursor-pointer hover:bg-[#FF7A3B] hover:text-white transition-all duration-300 ease-in-out ${
                  !isMobile() && !isTablet() ? "ml-[.625rem]" : ""
                } text-nowrap`}
              >
                Talk to us
              </div>
            ...
            <div
              className={`min-lg:hidden fixed ${
                isHamburgerMenu
                  ...
                  : "right-[.438rem]"
              } w-[2.75rem] h-[2.75rem] rounded-full ${
                isHamburgerMenu ? "bg-transparent" : "bg-[#FFFFFF1A]"
            ...
            >
              <div
                className={`flex flex-col gap-[0.438rem] ${
                  isHamburgerMenu ? "items-center" : "items-end"
                } justify-center transition-all duration-300 ease-in`}
              >
                <div
                  className={`w-[1.375rem] h-[0.125rem] bg-white transition-all duration-300 ease-in ${
                    isHamburgerMenu ? "absolute rotate-45" : ""
                  }`}
                />
                <div
                  className={`${
                    isHamburgerMenu ? "hidden" : ""
