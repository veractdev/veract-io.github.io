...
export default function Navbar() {
    ...
    return (
        <React.Fragment>
            {loaded && (
                <div className={`flex items-center justify-center ${isMobile() ? 'py-[0.625rem_0.375rem]' : `${isTablet() ? 'py-[0.625rem_0.375rem]' : 'py-[1.875rem_0.375rem]'}`}`}>
                    <div
                        className={`lg:w-[56.25rem] md:w-[40.813rem] w-[21.875rem] ${isHamburgerMenu ? 'h-max' : 'h-[3.5rem]'} rounded-[2.75rem] flex ${isHamburgerMenu ? 'flex-col' : 'flex-row'} items-center lg:justify-center bg-background ${isHamburgerMenu ? `${isMobile() ? 'py-[6.5rem_1.25rem]' : 'py-[4.125rem_1.25rem]'}` : 'p-[0.375rem_0.438rem_0.375rem_1.25rem]'} backdrop-blur-[0.563rem] transform transform-gpu transition-all duration-300 ease-in-out`}
                    >
                        {navItems.map((item) => (
                            <div
                                key={item.id}
                        ...
                        <div className={`${isHamburgerMenu ? `${isMobile() ? 'mt-[3.875rem]' : 'mt-[3.125rem]'}` : 'max-sm:hidden max-md:hidden max-lg:hidden'} navFont bg-white text-black font-bold leading-[1.2em] tracking-[0em] p-[0.563rem_2.063rem] rounded-[1.875rem] cursor-pointer hover:bg-[#FF7A3B] hover:text-white transition-all duration-300 ease-in-out ${!isMobile() && !isTablet() ? 'ml-[.625rem]' : ''} text-nowrap`}>Talk to us</div>
                        <div
