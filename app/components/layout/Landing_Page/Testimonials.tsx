  ...
  return (
    <div className='flex flex-row items-center justify-center gap-[2.75rem] p-[12.813rem_3.188rem_7.813rem_6.25rem]'>
      ...
      <div className='flex flex-row items-center justify-center gap-[1.125rem]'>
        ...
        >
          <img src="/Images/LandingPage/Testimonial/quotes.png" alt="testimonial quote" className={`${isActive ? '' : 'hidden'} w-[1.938rem] h-[1.438rem] absolute top-[6.688rem] left-[0.875rem]`} />
          <div className={`w-full flex flex-row items-center justify-center gap-[0.625rem] ${isActive ? 'pl-[1.313rem]' : ''}`}>
            <img
              src="/Images/LandingPage/Testimonial/testimonial_profile.png"
              alt="profile image"
              className={`${isActive ? '' : 'absolute bottom-[0.75rem] left-1/2 transform -translate-x-1/2'} w-[3.688rem] h-[3.688rem] rounded-full`}
            />
            <div className={`w-full flex flex-col ${isActive ? 'items-start' : 'items-center'} justify-center gap-[0.625rem]`}>
              <div className={`${isActive ? '' : 'absolute bottom-[12.25rem] transform -rotate-90 w-max'} interFont text-[1.25rem] text-[#E3E3E3] font-semibold leading-[1.2em]`}>Hare Priya Rajendran</div>
              <div className={`${isActive ? 'block' : 'hidden'} interFont text-[1.125rem] text-[#E3E3E3] font-normal leading-[1.2em]`}>Geeraj Business Solution</div>
            </div>
          </div>
          <div className={`${isActive ? '' : 'hidden'} px-[2.875rem_1.375rem] interFont text-[1rem] text-[#E3E3E3] font-normal leading-[1.3em]`}>
